"""
Hack script to prune directed cluster graph. Manual script for gold biovis edition.

WARNING: there seems to be a bug somewhere in this workflow!!!
"""
...
ADJ_FNAME = "data/biovis_gold/gold_0.32_dot_noweak_noclust.adj.csv"
RANKS_FNAME= "data/biovis_gold/gold_0.32_dot_noweak_ranks.txt"
IGNORE_FNAME= "data/biovis_gold/gold_0.32_dot_noweak_ignore_0.75.csv"
...
def load_ranks_named(fp, row_ids):
  """Load named ranks, return list of 0-index sets of integers from row_ids."""
  ranks = []
  for line in fp:
    line = line.strip()
    if not line: continue
    names = [s.strip() for s in line.split(',')]
    members = [row_ids.index(s) for s in names]
    ranks.append(sorted(members))
  return ranks
